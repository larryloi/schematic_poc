IF EXISTS (SELECT * FROM sys.objects o JOIN sys.schemas s ON o.schema_id = s.schema_id WHERE type = 'P' AND o.name = 'sp_user_operation_logsA' AND s.name = 'dbo')
BEGIN
    DROP PROCEDURE [dbo].[sp_user_operation_logsA]
END
GO
